...
- Combined CEMC+HCALOUT
- Standalone
- Output file - energy_verification_EtaCut_CircularCut_HCALOUT_CEMC.root
...
void LoopEval_CEMC_HCALOUT(int print = 1, int debug = 0, Double_t energyCutAggregate = 0.1, Double_t energyCut = 0.0, int trial = 0)
{

  Double_t EMC_cut = 0.2;

  TString detector = "HCALOUT_CEMC";
  TFile *f1 = new TFile("merged_Eval_HCALOUT.root","READ"); 
  TFile *f2 = new TFile("merged_Eval_CEMC.root","READ"); 
  ...
  Double_t y_radius_CEMC = 0.2; // Length of semi-major axis of circular (elliptical) in CEMC
  ...
  Double_t recalibration_firstSlice_HCALOUT = 0.0260;                             // Recalibration factor of first slice (needed to be done manually because of low statistics)
  Double_t recalibration_firstSlice_CEMC = 0.0300;
  ...
  //if(trial == 1){
    TH2D *te_minus_ge_by_ge_ge_EtaCut_CircularCut_Recalibrated_CEMC = new TH2D("te_minus_ge_by_ge_ge_EtaCut_CircularCut_Recalibrated_CEMC","#frac{#Delta e_{agg}}{truth e} vs truth e",nSlicesx,0,30,nSlicesy,te_minus_ge_by_ge_ge_min,te_minus_ge_by_ge_ge_max);
    TH2D *te_minus_ge_by_ge_ge_EtaCut_CircularCut_Recalibrated_HCALOUT = new TH2D("te_minus_ge_by_ge_ge_EtaCut_CircularCut_Recalibrated_HCALOUT","#frac{#Delta e_{agg}}{truth e} vs truth e",nSlicesx,0,30,nSlicesy,te_minus_ge_by_ge_ge_min,te_minus_ge_by_ge_ge_max);
    //}
  ...
  T1->SetBranchAddress("DST#EvalTTree_HCALOUT",&evaltree1);
  T2->SetBranchAddress("DST#EvalTTree_CEMC",&evaltree2);
  ...
  for(int i=0; i<T1->GetEntries(); i++) // We assume same no. of entries, since no cuts are applied
	...
	if(te_aggregate_CircularCut > energyCutAggregate && (te_aggregate_CircularCut - te_aggregate_HCALOUT_CircularCut) > EMC_cut){
	  ...
	  te_by_ge_ge_EtaCut_CircularCut_CEMC->Fill(ge, (te_aggregate_CircularCut - te_aggregate_HCALOUT_CircularCut)/ge);

	  mean_te_by_ge_ge_EtaCut_CircularCut_HCALOUT->Fill(ge, te_aggregate_HCALOUT_CircularCut/ge);
	  mean_te_by_ge_ge_EtaCut_CircularCut_CEMC->Fill(ge, (te_aggregate_CircularCut-te_aggregate_HCALOUT_CircularCut)/ge);
     
	  if(trial == 1){
	    te_minus_ge_by_ge_ge_EtaCut_CircularCut_Recalibrated_HCALOUT->Fill(ge, (te_aggregate_HCALOUT_CircularCut-ge)/ge);
	    te_minus_ge_by_ge_ge_EtaCut_CircularCut_Recalibrated_CEMC->Fill(ge, (te_aggregate_CircularCut - te_aggregate_HCALOUT_CircularCut-ge)/ge);
	  }
	  ...
	  if(debug==1){
	    ...
	    cout<<"(ge, (te_aggregate_CircularCut-te_aggregate_HCALOUT_CircularCut)/ge): ("<<ge<<", "<<(te_aggregate_CircularCut-te_aggregate_HCALOUT_CircularCut)/ge<<")\n"; 
	  }     
	} 
      }  
    }
  ...
  //recalibrationArr1[0] = recalibration_firstSlice_HCALOUT;
  //rf_integral_HCALOUT += recalibrationArr1[0];
  //cout << "Recalibration factor for slice " << 1 << " of HCALOUT is: " <<  recalibrationArr1[0] << endl;

  for(int binIter = 1; binIter <= nSlicesx; binIter++){
    recalibrationArr1[binIter-1] = mean_te_by_ge_ge_EtaCut_CircularCut_HCALOUT->GetBinContent(binIter);    
    rf_integral_HCALOUT += recalibrationArr1[binIter-1];

    if(debug == 1){
      cout<<"rf_integral_HCALOUT += "<<recalibrationArr1[binIter-1]<<"\n";
    }

    cout << "Recalibration factor for slice " << binIter << " of HCALOUT is: " <<  recalibrationArr1[binIter-1] << endl;
  }
  ...
  //recalibrationArr2[0] = recalibration_firstSlice_CEMC;
  //rf_integral_CEMC += recalibrationArr2[0];
  //cout << "Recalibration factor for slice " << 1 << " of CEMC is: " <<  recalibrationArr2[0] << endl;

  for(int binIter = 1; binIter <= nSlicesx; binIter++){
    recalibrationArr2[binIter-1] = mean_te_by_ge_ge_EtaCut_CircularCut_CEMC->GetBinContent(binIter);
    rf_integral_CEMC += recalibrationArr2[binIter-1];

    if(debug == 1){   
      cout<<"rf_integral_CEMC += "<<recalibrationArr2[binIter-1]<<"\n";
    }

    cout << "Recalibration factor for slice " << binIter << " of CEMC is: " <<  recalibrationArr2[binIter-1] << endl;
  }
  ...
  for(int i=0; i<T1->GetEntries(); i++){
	...
	if (twr1){ 
	  ...
	  Double_t te = twr1->get_te();
	    ...
	    if (pow(dphi/y_radius_HCALOUT,2)+pow(dtheta/x_radius_HCALOUT,2)<=1){
	      te_aggregate_CircularCut += te;
	      te_aggregate_HCALOUT_CircularCut += te;
	      te_aggregate_CircularCut_normalised += te*weight_HCALOUT/recalibrationArr1[recalibration_factor];
	    }
	  }
   	}
      }
	...
	if (twr2){ 
	  ...
	  Double_t te = twr2->get_te();
	    ...
	    if (pow(dphi/y_radius_CEMC,2)+pow(dtheta/x_radius_CEMC,2)<=1){
	      te_aggregate_CircularCut += te;
	      te_aggregate_CircularCut_normalised += te*weight_CEMC/recalibrationArr2[recalibration_factor];
	    }
	  }
   	}
      }
  ...
  if(trial == 1){
    ...
    slice7_HCALOUT = te_minus_ge_by_ge_ge_EtaCut_CircularCut_Recalibrated_HCALOUT->ProjectionY("slice7_HCALOUT", 7, 7);
    slice7_CEMC = te_minus_ge_by_ge_ge_EtaCut_CircularCut_Recalibrated_CEMC->ProjectionY("slice7_CEMC", 7, 7);

    slice7_HCALOUT->GetXaxis()->SetTitle("#Delta e^{agg}/ ge");
    slice7_HCALOUT->GetXaxis()->SetLabelSize(0.05);  
    slice7_HCALOUT->GetXaxis()->SetTitleSize(0.05);
    slice7_HCALOUT->GetYaxis()->SetTitle("Counts");
    slice7_HCALOUT->GetYaxis()->SetLabelSize(0.05);  
    slice7_HCALOUT->GetYaxis()->SetTitleSize(0.05);

    slice7_CEMC->GetXaxis()->SetTitle("#Delta e^{agg}/ ge");
    slice7_CEMC->GetXaxis()->SetLabelSize(0.05);  
    slice7_CEMC->GetXaxis()->SetTitleSize(0.05);
    slice7_CEMC->GetYaxis()->SetTitle("Counts");
    slice7_CEMC->GetYaxis()->SetLabelSize(0.05);  
    slice7_CEMC->GetYaxis()->SetTitleSize(0.05);

    slice7_HCALOUT -> Fit("fit", "M+");
    slice7_HCALOUT -> Draw("hist same");
    c->Print("HCALOUT_CEMC_sigmaE_slice7_HCALOUT.png");
   
    slice7_CEMC -> Fit("fit", "M+");
    slice7_CEMC -> Draw("hist same");
    c->Print("HCALOUT_CEMC_sigmaE_slice7_CEMC.png");
